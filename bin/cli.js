...
import { syncAllHooks } from '../lib/hooks.js';
import { RUNTIMES, getRulesPath, writeIntegrationFile } from '../lib/runtimes.js';
import { removeHooksFromRuntime, readHooksManifest } from '../lib/hooks.js';
...
program
  ...
  .action((opts) => {
    ...
    console.log(`\nContext Budget: ${report.budget.total}/150 lines ${report.budget.overBudget ? '❌' : '✅'}`);
  ...
  .action(async (opts) => {
    ...
    if (opts.module && (opts.enable || opts.disable)) {
      ...
      if (opts.local || opts.personal) {
        ...
        console.log(`${opts.module} ${enabled ? 'enabled' : 'disabled'} globally`);
      }
      return;
    }
    ...
    for (const m of mods) {
      ...
      if (m.enabled !== shouldBeEnabled) {
        ...
        console.log(`${m.name}: ${shouldBeEnabled ? 'enabled' : 'disabled'}`);
      }
    }
  });
...
program
  ...
  .action(async (opts) => {
    ...
    for (const moduleName of Object.keys(manifest)) {
      removeHooksFromRuntime(moduleName, config.runtimes || [], gumDir, homeDir);
    }
